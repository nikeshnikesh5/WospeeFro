...
const UserProfilePageComponent = ({ updateUserApiRequest, fetchUser, userInfoFormRedux,
setReduxUserState, reduxDispatch , localStorage ,sessionStorage }) => {
  ...
  const userInfo = userInfoFormRedux
  ...
  const handleSubmit = (event) => {
    ...
    if (event.currentTarget.checkValidity() === true && form.password.value === form.confirmPassword.value) {
        updateUserApiRequest(name, lastName, phoneNumber, address, country, zipCode, city, state, password).then(data => {
           ...
           reduxDispatch(setReduxUserState({doNotLogout: userInfo.doNotLogout,
          ...data.userUpdated
          }));
          if(userInfo.doNotLogout) localStorage.setItem("userInfo",JSON.
          stringify({doNotLogout:true, ...data.userUpdated}));
          else sessionStorage.setItem("userInfo",JSON.stringify({
            doNotLogout:false, ...data.userUpdated
          }));
